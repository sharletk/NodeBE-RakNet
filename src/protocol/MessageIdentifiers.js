const MessageIdentifiers = {};
	...
	/// If protocol fails on server, returns ID_INCOMPATIBLE_PROTOCOL_VERSION to client
	 MessageIdentifiers.ID_OPEN_CONNECTION_REQUEST_1 = 0x05;
	...
	/// , public key (if do security is true), MTU(2). If public key fails on client, returns ID_PUBLIC_KEY_MISMATCH
	 MessageIdentifiers.ID_OPEN_CONNECTION_REPLY_1 = 0x06;
	...
	/// Second byte indicates type. Used currently for NAT punchthrough for receiver port advertisement. See ID_NAT_ADVERTISE_RECIPIENT_PORT
	 MessageIdentifiers.ID_OUT_OF_BAND_INTERNAL = 0x0d;
	...
	/// the message arrived
	 MessageIdentifiers.ID_SND_RECEIPT_ACKED = 0x0e;
	...
	/// ID_SND_RECEIPT_LOSS for unsent messages, you should consider those messages as all lost.
	 MessageIdentifiers.ID_SND_RECEIPT_LOSS = 0x0f;
	...
	// The 2nd byte of the message contains the value of RAKNET_PROTOCOL_VERSION for the remote system
	 MessageIdentifiers.ID_INCOMPATIBLE_PROTOCOL_VERSION = 0x19;
	...
	/// RakPeer::SetLimitIPConnectionFrequency()
	 MessageIdentifiers.ID_IP_RECENTLY_CONNECTED = 0x1a;
	...
	/// in system times between the sender and the recipient. Requires that you call SetOccasionalPing.
	 MessageIdentifiers.ID_TIMESTAMP = 0x1b;
	...
	/// Read using bitstreams
	 MessageIdentifiers.ID_UNCONNECTED_PONG = 0x1c;
	...
	/// the data parameter
	 MessageIdentifiers.ID_ADVERTISE_SYSTEM = 0x1d;
	...
	///  and partLength in OnFileProgress in FileListTransferCBInterface.h
	 MessageIdentifiers.ID_DOWNLOAD_PROGRESS = 0x1e;
	...
	///   Packet::systemAddress is modified to reflect the systemAddress of this client.
	 MessageIdentifiers.ID_REMOTE_DISCONNECTION_NOTIFICATION = 0x1f;
	/// ConnectionGraph2 plugin - In a client/server environment, a client other than ourselves has been forcefully dropped.
	///  Packet::systemAddress is modified to reflect the systemAddress of this client.
	 MessageIdentifiers.ID_REMOTE_CONNECTION_LOST = 0x20;
	/// ConnectionGraph2 plugin: Bytes 1-4 = count. for (count items) contains {SystemAddress, RakNetGUID, 2 byte ping}
	 MessageIdentifiers.ID_REMOTE_NEW_INCOMING_CONNECTION = 0x21;

	/// FileListTransfer plugin - Setup data
	 MessageIdentifiers.ID_FILE_LIST_TRANSFER_HEADER = 0x22;
	/// FileListTransfer plugin - A file
	 MessageIdentifiers.ID_FILE_LIST_TRANSFER_FILE = 0x23;
	// Ack for reference push, to send more of the file
	 MessageIdentifiers.ID_FILE_LIST_REFERENCE_PUSH_ACK = 0x24;

	/// DirectoryDeltaTransfer plugin - Request from a remote system for a download of a directory
	 MessageIdentifiers.ID_DDT_DOWNLOAD_REQUEST = 0x25;

	/// RakNetTransport plugin - Transport provider message, used for remote console
	 MessageIdentifiers.ID_TRANSPORT_STRING = 0x26;

	/// ReplicaManager plugin - Create an object
	 MessageIdentifiers.ID_REPLICA_MANAGER_CONSTRUCTION = 0x27;
	/// ReplicaManager plugin - Changed scope of an object
	 MessageIdentifiers.ID_REPLICA_MANAGER_SCOPE_CHANGE = 0x28;
	/// ReplicaManager plugin - Serialized data of an object
	 MessageIdentifiers.ID_REPLICA_MANAGER_SERIALIZE = 0x29;
	/// ReplicaManager plugin - New connection, about to send all world objects
	 MessageIdentifiers.ID_REPLICA_MANAGER_DOWNLOAD_STARTED = 0x2a;
	/// ReplicaManager plugin - Finished downloading all serialized objects
	 MessageIdentifiers.ID_REPLICA_MANAGER_DOWNLOAD_COMPLETE = 0x2b;

	/// RakVoice plugin - Open a communication channel
	 MessageIdentifiers.ID_RAKVOICE_OPEN_CHANNEL_REQUEST = 0x2c;
	/// RakVoice plugin - Communication channel accepted
	 MessageIdentifiers.ID_RAKVOICE_OPEN_CHANNEL_REPLY = 0x2d;
	/// RakVoice plugin - Close a communication channel
	 MessageIdentifiers.ID_RAKVOICE_CLOSE_CHANNEL = 0x2e;
	/// RakVoice plugin - Voice data
	 MessageIdentifiers.ID_RAKVOICE_DATA = 0x2f;

	/// Autopatcher plugin - Get a list of files that have changed since a certain date
	 MessageIdentifiers.ID_AUTOPATCHER_GET_CHANGELIST_SINCE_DATE = 0x30;
	/// Autopatcher plugin - A list of files to create
	 MessageIdentifiers.ID_AUTOPATCHER_CREATION_LIST = 0x31;
	/// Autopatcher plugin - A list of files to delete
	 MessageIdentifiers.ID_AUTOPATCHER_DELETION_LIST = 0x32;
	/// Autopatcher plugin - A list of files to get patches for
	 MessageIdentifiers.ID_AUTOPATCHER_GET_PATCH = 0x33;
	/// Autopatcher plugin - A list of patches for a list of files
	 MessageIdentifiers.ID_AUTOPATCHER_PATCH_LIST = 0x34;
	/// Autopatcher plugin - Returned to the user: An error from the database repository for the autopatcher.
	 MessageIdentifiers.ID_AUTOPATCHER_REPOSITORY_FATAL_ERROR = 0x35;
	/// Autopatcher plugin - Returned to the user: The server does not allow downloading unmodified game files.
	 MessageIdentifiers.ID_AUTOPATCHER_CANNOT_DOWNLOAD_ORIGINAL_UNMODIFIED_FILES = 0x36;
	/// Autopatcher plugin - Finished getting all files from the autopatcher
	 MessageIdentifiers.ID_AUTOPATCHER_FINISHED_INTERNAL = 0x37;
	 MessageIdentifiers.ID_AUTOPATCHER_FINISHED = 0x38;
	/// Autopatcher plugin - Returned to the user: You must restart the application to finish patching.
	 MessageIdentifiers.ID_AUTOPATCHER_RESTART_APPLICATION = 0x39;

	/// NATPunchthrough plugin: internal
	 MessageIdentifiers.ID_NAT_PUNCHTHROUGH_REQUEST = 0x3a;
	/// NATPunchthrough plugin: internal
	//ID_NAT_GROUP_PUNCHTHROUGH_REQUEST,
	/// NATPunchthrough plugin: internal
	//ID_NAT_GROUP_PUNCHTHROUGH_REPLY,
	/// NATPunchthrough plugin: internal
	 MessageIdentifiers.ID_NAT_CONNECT_AT_TIME = 0x3b;
	/// NATPunchthrough plugin: internal
	 MessageIdentifiers.ID_NAT_GET_MOST_RECENT_PORT = 0x3c;
	/// NATPunchthrough plugin: internal
	 MessageIdentifiers.ID_NAT_CLIENT_READY = 0x3d;
	/// NATPunchthrough plugin: internal
	...
	///  RakNetGUID destination field of NatPunchthroughClient::OpenNAT().
	 MessageIdentifiers.ID_NAT_TARGET_NOT_CONNECTED = 0x3e;
	...
	///  Bytes starting at offset 1 contains the RakNetGUID  destination field of NatPunchthroughClient::OpenNAT().
	 MessageIdentifiers.ID_NAT_TARGET_UNRESPONSIVE = 0x3f;
	...
	///  field of NatPunchthroughClient::OpenNAT().
	 MessageIdentifiers.ID_NAT_CONNECTION_TO_TARGET_LOST = 0x40;
	/// NATPunchthrough plugin: This punchthrough is already in progress. Possibly the plugin is not installed.
	///  Bytes starting at offset 1 contains the RakNetGUID destination field of NatPunchthroughClient::OpenNAT().
	 MessageIdentifiers.ID_NAT_ALREADY_IN_PROGRESS = 0x41;
	...
	///  packet::guid contains the destination field of NatPunchthroughClient::OpenNAT(). Byte 1 contains 1 if you are the sender, 0 if not
	 MessageIdentifiers.ID_NAT_PUNCHTHROUGH_FAILED = 0x42;
	...
	/// ReadyEvent plugin - Request of ready event state - used for pulling data when newly connecting
	 MessageIdentifiers.ID_READY_EVENT_QUERY = 0x47;

	/// Lobby packets. Second byte indicates type.
	 MessageIdentifiers.ID_LOBBY_GENERAL = 0x48;

	// RPC3, RPC4 error
	 MessageIdentifiers.ID_RPC_REMOTE_ERROR = 0x49;
	/// Plugin based replacement for RPC system
	 MessageIdentifiers.ID_RPC_PLUGIN = 0x4a;

	/// FileListTransfer transferring large files in chunks that are read only when needed, to save memory
	 MessageIdentifiers.ID_FILE_LIST_REFERENCE_PUSH = 0x4b;
	/// Force the ready event to all set
	 MessageIdentifiers.ID_READY_EVENT_FORCE_ALL_SET = 0x4c;

	/// Rooms function
	 MessageIdentifiers.ID_ROOMS_EXECUTE_FUNC = 0x4d;
	 MessageIdentifiers.ID_ROOMS_LOGON_STATUS = 0x4e;
	 MessageIdentifiers.ID_ROOMS_HANDLE_CHANGE = 0x4f;

	/// Lobby2 message
	 MessageIdentifiers.ID_LOBBY2_SEND_MESSAGE = 0x50;
	 MessageIdentifiers.ID_LOBBY2_SERVER_ERROR = 0x51;
	...
	/// If the oldHost is UNASSIGNED_RAKNET_GUID, then this is the first time the host has been determined
	 MessageIdentifiers.ID_FCM2_NEW_HOST = 0x52;
	/// \internal For FullyConnectedMesh2 plugin
	 MessageIdentifiers.ID_FCM2_REQUEST_FCMGUID = 0x53;
	/// \internal For FullyConnectedMesh2 plugin
	 MessageIdentifiers.ID_FCM2_RESPOND_CONNECTION_COUNT = 0x54;
	/// \internal For FullyConnectedMesh2 plugin
	 MessageIdentifiers.ID_FCM2_INFORM_FCMGUID = 0x55;
	/// \internal For FullyConnectedMesh2 plugin
	 MessageIdentifiers.ID_FCM2_UPDATE_MIN_TOTAL_CONNECTION_COUNT = 0x56;
	...
	/// \sa FullyConnectedMesh2::StartVerifiedJoin()
	 MessageIdentifiers.ID_FCM2_VERIFIED_JOIN_START = 0x57;
	/// \internal The client has completed processing for all systems designated in ID_FCM2_VERIFIED_JOIN_START
	 MessageIdentifiers.ID_FCM2_VERIFIED_JOIN_CAPABLE = 0x58;
	...
	/// \note Only the designated client gets this message
	 MessageIdentifiers.ID_FCM2_VERIFIED_JOIN_FAILED = 0x59;
	...
	/// \sa RespondOnVerifiedJoinCapable()
	 MessageIdentifiers.ID_FCM2_VERIFIED_JOIN_ACCEPTED = 0x5a;
	...
	/// \note Only the designated client gets this message
	/// \sa RespondOnVerifiedJoinCapable()
	 MessageIdentifiers.ID_FCM2_VERIFIED_JOIN_REJECTED = 0x5b;

	/// UDP proxy messages. Second byte indicates type.
	 MessageIdentifiers.ID_UDP_PROXY_GENERAL = 0x5c;

	/// SQLite3Plugin - execute
	 MessageIdentifiers.ID_SQLite3_EXEC = 0x5d;
	/// SQLite3Plugin - Remote database is unknown
	 MessageIdentifiers.ID_SQLite3_UNKNOWN_DB = 0x5e;
	/// Events happening with SQLiteClientLoggerPlugin
	 MessageIdentifiers.ID_SQLLITE_LOGGER = 0x5f;

	/// Sent to NatTypeDetectionServer
	 MessageIdentifiers.ID_NAT_TYPE_DETECTION_REQUEST = 0x60;
	/// Sent to NatTypeDetectionClient. Byte 1 contains the type of NAT detected.
	 MessageIdentifiers.ID_NAT_TYPE_DETECTION_RESULT = 0x61;

	/// Used by the router2 plugin
	 MessageIdentifiers.ID_ROUTER_2_INTERNAL = 0x62;
	...
	/// Packet::guid contains the endpoint guid that we were trying to reach
	 MessageIdentifiers.ID_ROUTER_2_FORWARDING_NO_PATH = 0x63;
	...
	/// rakPeerInterface->Connect(ipAddressString, sourceToDestPort, 0,0);
	 MessageIdentifiers.ID_ROUTER_2_FORWARDING_ESTABLISHED = 0x64;
	...
	/// Read endpointGuid and port as per ID_ROUTER_2_FORWARDING_ESTABLISHED
	 MessageIdentifiers.ID_ROUTER_2_REROUTED = 0x65;

	/// \internal Used by the team balancer plugin
	 MessageIdentifiers.ID_TEAM_BALANCER_INTERNAL = 0x66;
	...
	/// For TeamBalancer: Byte 1 contains the team you requested to join. Following bytes contain NetworkID of which member
	 MessageIdentifiers.ID_TEAM_BALANCER_REQUESTED_TEAM_FULL = 0x67;
	...
	/// For TeamBalancer: Byte 1 contains the team you requested to join.
	 MessageIdentifiers.ID_TEAM_BALANCER_REQUESTED_TEAM_LOCKED = 0x68;
	 MessageIdentifiers.ID_TEAM_BALANCER_TEAM_REQUESTED_CANCELLED = 0x69;
	/// Team balancer plugin informing you of your team. Byte 1 contains the team you requested to join. Following bytes contain NetworkID of which member.
	 MessageIdentifiers.ID_TEAM_BALANCER_TEAM_ASSIGNED = 0x6a;

	/// Gamebryo Lightspeed integration
	 MessageIdentifiers.ID_LIGHTSPEED_INTEGRATION = 0x6b;

	/// XBOX integration
	 MessageIdentifiers.ID_XBOX_LOBBY = 0x6c;
	...
	/// RakNet::BitStream bs(packet->data, packet->length, false); bs.IgnoreBytes(sizeof(RakNet::MessageID)); RakNet::RakString password; bs.Read(password);
	 MessageIdentifiers.ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_SUCCESS = 0x6d;
	 MessageIdentifiers.ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_SUCCESS = 0x6e;
	...
	/// If the other system must pass the challenge to stay connected, you should call RakPeer::CloseConnection() to terminate the connection to the other system.
	 MessageIdentifiers.ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_FAILURE = 0x6f;
	...
	/// RakNet::BitStream bs(packet->data, packet->length, false); bs.IgnoreBytes(sizeof(MessageID)); RakNet::RakString password; bs.Read(password);
	 MessageIdentifiers.ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILURE = 0x70;
	...
	/// You can read the identifier used to challenge as follows:
	/// RakNet::BitStream bs(packet->data, packet->length, false); bs.IgnoreBytes(sizeof(MessageID)); RakNet::RakString password; bs.Read(password);
	 MessageIdentifiers.ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_TIMEOUT = 0x71;
	/// \internal
	 MessageIdentifiers.ID_TWO_WAY_AUTHENTICATION_NEGOTIATION = 0x72;

	/// CloudClient / CloudServer
	 MessageIdentifiers.ID_CLOUD_POST_REQUEST = 0x73;
	 MessageIdentifiers.ID_CLOUD_RELEASE_REQUEST = 0x74;
	 MessageIdentifiers.ID_CLOUD_GET_REQUEST = 0x75;
	 MessageIdentifiers.ID_CLOUD_GET_RESPONSE = 0x76;
	 MessageIdentifiers.ID_CLOUD_UNSUBSCRIBE_REQUEST = 0x77;
	 MessageIdentifiers.ID_CLOUD_SERVER_TO_SERVER_COMMAND = 0x78;
	 MessageIdentifiers.ID_CLOUD_SUBSCRIPTION_NOTIFICATION = 0x79;

	// LibVoice
	 MessageIdentifiers.ID_LIB_VOICE = 0x7a;

	 MessageIdentifiers.ID_RELAY_PLUGIN = 0x7b;
	 MessageIdentifiers.ID_NAT_REQUEST_BOUND_ADDRESSES = 0x7c;
	 MessageIdentifiers.ID_NAT_RESPOND_BOUND_ADDRESSES = 0x7d;
	 MessageIdentifiers.ID_FCM2_UPDATE_USER_CONTEXT = 0x7e;
	 MessageIdentifiers.ID_RESERVED_3 = 0x7f;
	 MessageIdentifiers.ID_RESERVED_4 = 0x80;
	 MessageIdentifiers.ID_RESERVED_5 = 0x81;
	 MessageIdentifiers.ID_RESERVED_6 = 0x82;
	 MessageIdentifiers.ID_RESERVED_7 = 0x83;
	 MessageIdentifiers.ID_RESERVED_8 = 0x84;
	 MessageIdentifiers.ID_RESERVED_9 = 0x85;

	// For the user to use.  Start your first enumeration at this value.
	 MessageIdentifiers.ID_USER_PACKET_ENUM = 0x86;
	//-------------------------------------------------------------------------------------------------------------
